...
function drawCharacterSelect() {
    background(pacManBlack);
    // Buttons removed
    if (!isMuted && !selectMusicStarted && musicSelect?.isLoaded()) { musicSelect.loop(); selectMusicStarted = true; currentAttributionText = selectAttribution;}

    // --- How to Play Box ---
    let instructionY = 100;
    let instructionWidth = width * 0.65;
    let instructionSize = 16; // <<< Increased size
    let lineSpacing = 22;    // <<< Increased spacing
    let instructions = "HOW TO PLAY\n\nSmashHead needs your help! Smack the head with the hammer to bounce around.\nHit all the dots before time runs out.\nMore smacks = more chaos = more wins.";
    let hammerScale = 0.6; if(!hammerDisplayWidth || !hammerDisplayHeight) calculateImageSize(); let hammerW = (hammerDisplayWidth || 100) * hammerScale; let hammerH = (hammerDisplayHeight || 150) * hammerScale;
    let totalBlockWidth = instructionWidth + hammerW + 40; let blockStartX = (width - totalBlockWidth) / 2;
    let imgX = blockStartX + hammerW / 2; let imgY = instructionY + hammerH * 0.6; let textX = imgX + hammerW / 2 + 20;
    let framePadding = 20; let frameX = blockStartX - framePadding / 2; let frameY = instructionY - framePadding * 1.5;
    let lines = instructions.split('\n'); let estimatedLines = lines.length; estimatedLines += Math.ceil(instructions.length / (instructionWidth / (instructionSize * 0.6))); let estTextHeight = estimatedLines * lineSpacing + 30; // Added more buffer for bigger text
    let frameHeight = max(hammerH + (imgY - instructionY) + framePadding, estTextHeight) + framePadding * 2; let frameWidth = totalBlockWidth + framePadding;

    // Draw outer frame
    noFill(); stroke(pacManCyan); strokeWeight(4); rect(frameX, frameY, frameWidth, frameHeight); noStroke();
    ...
    if (retroFont) textFont(retroFont); else textFont('monospace'); fill(pacManYellow); textAlign(LEFT, TOP); textSize(instructionSize); textLeading(lineSpacing); text(instructions, textX, instructionY, instructionWidth);

    // Draw Hammer Image
    if (hammerImg && typeof hammerDisplayHeight === 'number') { push(); translate(imgX, imgY); rotate(-PI / 6); imageMode(CENTER); image(hammerImg, 0, 0, hammerW, hammerH); pop(); imageMode(CENTER); }

    // --- Pick Your Head Section ---
    let selectY = height * 0.70; // Needs enough space below instructions
    let spacing = width / 4;
    // btnY removed
    let btnWidth = 80; // Keep for potential future use? (not used now)
    let headNames = ["Ol' Greenie", "Blurg", "Frank"];
    let pickTextY = frameY + frameHeight + 50; // Position below instruction frame
    selectHue = (selectHue + 1.5) % 360; if (retroFont) textFont(retroFont); else textFont('monospace'); textAlign(CENTER, CENTER); textSize(26); push(); colorMode(HSB, 360, 100, 100, 100); fill(selectHue, 90, 100); text("Pick your head to SMASH!", width / 2, pickTextY); pop(); colorMode(RGB, 255, 255, 255, 255);

    // Draw heads, hover box, and names below
    // Changed alignment to CENTER, TOP for names below
    textAlign(CENTER, TOP); textSize(16);
    for (let i = 0; i < 3; i++) {
        let displayImg = normalHeadImages[i]; let currentSelectHeadSize = typeof selectHeadSize === 'number' ? selectHeadSize : 90; let imgXPos = spacing * (i+1); let currentHeadHeight = currentSelectHeadSize; let aspect = 1;
         if (displayImg && displayImg.width > 0) { aspect = displayImg.height / displayImg.width || 1; currentHeadHeight = currentSelectHeadSize * aspect; }
        // Draw hover box
        if (i === hoveredHeadIndex) { let boxPadding = 5; let boxW = currentSelectHeadSize + 2 * boxPadding; let boxH = currentHeadHeight + 2 * boxPadding; let boxX = imgXPos - boxW / 2; let boxY = selectY - boxH / 2; stroke(pacManYellow); strokeWeight(3); noFill(); rect(boxX, boxY, boxW, boxH); noStroke(); }
        // Draw head image or placeholder
        if (displayImg && displayImg.width > 0) { image(displayImg, imgXPos, selectY, currentSelectHeadSize, currentHeadHeight); }
        else { fill(pacManRed); rect(imgXPos-currentSelectHeadSize/2, selectY-currentSelectHeadSize/2, currentSelectHeadSize, currentSelectHeadSize); }
        // <<< Draw Name Below >>>
        fill(pacManYellow);
        text(headNames[i], imgXPos, selectY + currentHeadHeight*0.5 + 10); // Positioned below + padding
        // Buttons removed
    }
}
